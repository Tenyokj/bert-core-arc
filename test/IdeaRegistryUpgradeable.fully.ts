...
describe("IdeaRegistry", function () {
  ...
  describe("Creation", function () {
    it("uses a sane USDC-native default author stake", async function () {
      const { admin, reputationSystem, voterProgression, roles, ethers } =
        await deploySystem();
      ...
      expect(await ideaRegistry.authorMinStake()).to.equal(
    ...
    it("should validate token balance and allowance before locking author stake", async function () {
      const { user1, ideaRegistry, fundingPool, usdc } = await deploySystem();

      const userBalance = await usdc.balanceOf(user1.address);
      const aboveBalance = userBalance + 1n;
